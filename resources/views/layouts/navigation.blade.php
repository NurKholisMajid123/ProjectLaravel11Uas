<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
            ...
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @if(Auth::user()->usertype == 'admin')
                        ...
                        <x-nav-link :href="url('/m')" :active="request()->routeIs('admin.category')">
                            {{ __('Category') }}
                        </x-nav-link>

                        <x-nav-link :href="url('/m')" :active="request()->routeIs('admin.user')">
                            {{ __('User') }}
                        </x-nav-link>
                    @endif

                    @if(Auth::user()->usertype == 'user')
                        <x-nav-link :href="url('/user/product')" :active="request()->routeIs('user.order')">
                            {{ __('Order') }}
                        </x-nav-link>

                        <x-nav-link :href="url('/favorite')" :active="request()->routeIs('user.favorite')">
                            {{ __('Favorite') }}
                        </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if(Auth::user()->usertype == 'admin')
                        <x-responsive-nav-link :href="url('/m')" :active="request()->routeIs('admin.product')">
                            {{ __('Product') }}
                        </x-responsive-nav-link>

                        <x-responsive-nav-link :href="url('/m')" :active="request()->routeIs('admin.category')">
                            {{ __('Category') }}
                        </x-responsive-nav-link>

                        <x-responsive-nav-link :href="url('/m')" :active="request()->routeIs('admin.user')">
                            {{ __('User') }}
                        </x-responsive-nav-link>
                    @endif

                    @if(Auth::user()->usertype == 'user')
                        <x-responsive-nav-link :href="url('/')" :active="request()->routeIs('user.order')">
                            {{ __('Order') }}
                        </x-responsive-nav-link>
